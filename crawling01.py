...
def crawler(key_word):
    with open('all_case.csv', 'w', newline='') as csvfile:
        cases = csv.writer(csvfile, delimiter=',')
        row = ['case_id', 'institute', 'program', 'degree', 'season', 'decision', 'notification', 'result_date', 'undergrad gpa', 'gre_v', 'gre_q', 'gre_w', 'gre_subject', 'status', 'submit_date', 'notes']
        cases.writerow(row)
        ...
        for i in range(1, page_num+1):
            ...
            for tr in tr_list:
                ...
                if tr_id not in case_id:
                    institute, program, degree, season, decision,notification, result_date, gpa, gre_v, gre_q, gre_w, gre_subject, status, submit_date, notes = tuple([None]*15)
                    case_id |= {tr_id}
                    ...
                    if td_list[2].find_all('a'):
                        ...
                        # column 13
                        gre_subject = gpa_score.find_all('strong')[2].next_sibling[2:]
                    # column 14
                    ...
                    row = [tr_id, institute, program, degree, season, decision,notification, result_date, gpa, gre_v, gre_q, gre_w, gre_subject, status, submit_date, notes]
