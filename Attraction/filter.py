...
def crawl_page():
    ...
    for ct in ct_list:
        ...
        print(ct_url)
        explore_city(ct_url, limiting_domain, attraction_dict, is_good)

    return attraction_dict
    #return is_good


def explore_city(city_url, limiting_domain, attraction_dict, is_good):
    ...
    if page_soup.find('div', class_='unified pagination '):
        ...
        while not page_soup.find_all('span', class_ ='nav next disabled'):
            add_dictionary(attraction_dict, page_soup, limiting_domain, city_url, is_good)
        ...
        add_dictionary(attraction_dict, page_soup, limiting_domain, city_url, is_good)
    #return attraction_dict ### DELETE it!!!!!!
    #return is_good



def add_dictionary(attraction_dict, page_soup, limiting_domain, city_url,is_good):
    count = 0
        ...
        if name in attraction_dict:
            ...
            if review_tag.find('a'):
                ...
                if review_count < 1000:
                    ...
                    #is_good.append(listing.find("a").get("href"))
                    url = limiting_domain + listing.find("a").get("href")
                    print(review_count)
                    attraction_dict[name] = url
                    #break
            #is_good.append(city_url) #record cities
